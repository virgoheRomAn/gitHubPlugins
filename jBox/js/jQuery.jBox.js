;(function ($, win) {
    $.jBox = function () {
        ...
        function setBoxLayout(box, opt) {
            ...
            //给 标题/内容/按钮 添加样式名称
            $box.find(".jBox-title").addClass(elementCls.titleCls);
            $box.find(".jBox-cont").addClass(elementCls.contCls);
            $box.find(".jBox-btn").addClass(elementCls.btnCls);

            //元素添加css样式
            $box.find(".jBox-title").css(elementCSS.titleCSS);  // 标题
            $box.find(".jBox-title > label").css(elementCSS.titleTextCSS);  // 标题文字
            $box.find(".jBox-title > .jBox-close-btn").css(elementCSS.closeCSS);  // 关闭按钮
            $box.find(".jBox-cont").css(elementCSS.contCSS);  // 内容
            $box.find(".jBox-cont  .jBox-intro").css(elementCSS.introCSS);  //内容详包含图标情
            $box.find(".jBox-cont  .jBox-intro-cont").css(elementCSS.introContCSS);  // 内容文字详情
            $box.find(".jBox-cont  .jBox-icon-box").css(elementCSS.iconCSS);  // 内容图标详情
            $box.find(".jBox-btn").css(elementCSS.btnCSS);  // 按钮

            //给按钮添加样式
            $box.find(".jBox-btn a").each(function (i) {
                $(this).css(!btn.array[i].css ? {} : btn.array[i].css).click(function () {
            ...
            if (icon.iconType !== "none" && icon.iconDir.toString().toLowerCase() === "h") {
                $box.find(".jBox-cont  .jBox-intro-cont").css("padding-left", !icon.width ? 0 : parseInt(icon.width) + 10);
            }
            ...
            if (hasHeight) {
                if (!opt.showType) {
                    $box.find(".jBox-cont").addClass("jBox-height");
                } else {
                    $box.find(".jBox-intro").height(parseInt(elementOpt.height) - $box.find(".jBox-title").outerHeight(true) - $box.find(".jBox-btn").outerHeight(true));
                }
                $box.find(".jBox-intro").addClass("jBox-table").find(".jBox-intro-cont").addClass("jBox-cell");
            }
            ...
            if (hasWidth && !hasHeight) {
                ...
                //同时设置高宽
                new_width = $box.outerWidth(true);
                new_height = isVertical ? elementOpt.height : "auto";
            } else {
                //都没设置高宽
                //+1解决中英文在一起出现的问题
